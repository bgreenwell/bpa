...
The most basic function is `get_pattern`, which is not exported so one must use `bpa:::get_pattern`. This function is used internally to extract patterns from a vector of data. For example, consider `Date` column. We can easily extract the unique patterns:
```{r}
patterns <- bpa:::get_pattern(messy$Date)
...
# Extract Gender values matching a specific pattern
